...
export const useOnStepXStore = defineStore('onstepx', () => {
    const firmwareVersion = ref('-')
    const comPort = ref('-')
    const isConnected = ref(false)
    const connectionStatus = ref('Disconnected')
    const proxyVersion = ref('')
    const position = ref({ ra: 0, dec: 0, alt: 0, az: 0 })
/*
...
*/
    ...
    async function fetchProxyVersion() {
        try {
            const response = await fetch('/ui/proxy/version')
            if (response.ok) {
    ...
    async function fetchFirmwareVersion() {
        try {
            const response = await fetch('/ui/firmware/version')
            if (response.ok) {
                const data = await response.json()
    ...
    async function fetchPosition() {
        try {
            const response = await fetch('/ui/position')
            if (response.ok) {
                const data = await response.json()
                position.value = data
            }
